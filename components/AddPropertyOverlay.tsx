...
export function AddPropertyOverlay() {
  const { open, editId, closeOverlay, blurTargetRef } = useAddPropertyOverlay();

  if (!open) return null;
  return <AddPropertyOverlayContent key={editId ?? "new"} editId={editId} closeOverlay={closeOverlay} blurTargetRef={blurTargetRef} />;
}
...
function AddPropertyOverlayContent({ editId, closeOverlay, blurTargetRef }: ContentProps) {
  ...
  const blurProps =
    Platform.OS === "android"
      ? { blurMethod: "dimezisBlurView" as const, blurTarget: blurTargetRef }
      : {};

  return (
    <View style={StyleSheet.absoluteFill} pointerEvents="box-none">
      <SafeAreaView className="flex-1">
        <BlurView intensity={65} tint="dark" pointerEvents="none" className="absolute inset-0" {...blurProps} />
        <View pointerEvents="none" className="absolute inset-0 bg-black/35" />
        <Pressable className="flex-1 items-center justify-center px-3 py-3" onPress={closeOverlay}>
          <Pressable
          ...
          >
            <View className="flex-row items-center justify-between border-b border-outline-variant/20 px-6 py-4">
              ...
              <Pressable className="h-10 w-10 items-center justify-center rounded-full bg-surface-container-low" onPress={closeOverlay}>
            ...
            <View className="border-t border-outline-variant/20 bg-white/95 px-6 py-4">
              ...
              <View className="flex-row gap-3">
                <Pressable className="flex-1 items-center rounded-xl bg-surface-container-highest py-4" onPress={closeOverlay} disabled={saving}>
